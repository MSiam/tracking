...
void KCFTracker::preprocess(Mat imgOrig, Point centroid, int w, int h)
{
	...
	tSetup.min_scale_factor = pow((double)tParams.scale_step, ceil(log(max(5.0 / tSetup.padded.width, 5.0 / tSetup.padded.height)) / log(tParams.scale_step))); tSetup.max_scale_factor = pow(tParams.scale_step, floor(log(min((float) rows / tSetup.original.height, (float) cols / tSetup.original.width)) / log(tParams.scale_step))); }, scalingParam);
