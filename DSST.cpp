...
public:
	...
	Mat *create_feature_map(Mat& patch, int full, int &nChns, Mat& Gray,
			bool scaling)
	{
		...
		if (!scaling)
		{
			binSize = hParams.transBinSize;
		}
		else
		{
			binSize = hParams.scaleBinSize;
		}
		int hb = (float) h / binSize; //ceil((float) h / binSize);
		int wb = (float) w / binSize; //ceil((float) w / binSize);
	...
	Mat get_scale_sample(Mat img, trackingSetup tSetup, Params tParams,
			int &nDims, bool display = false)
	{
		Mat featureMapScale;
		CvRect patchSize;
	...
	void preprocess(int rows, int cols, cv::Mat img, int left, int top,
			int right, int bottom, VOTPolygon p)
	{
		...
		int szPadding_w = (float) tSetup.padded.width / hParams.transBinSize;
		int szPadding_h = (float) tSetup.padded.height / hParams.transBinSize;
	...
	cv::Mat visualize(Rect rect, cv::Mat img, Scalar scalar = Scalar(255, 0, 0))
	{
...
int main()
{
	generateFiles("/media/New Volume/Scene_DS/VOT/ball/");
	cv::Mat initFrame;
	...
	while (true)
	{
		...
		cv::Rect rect;
		double time;
		timeOfBlock( rect = dsst.processFrame(currentFrame, dsst.tSetup.enableScale);, time);
		totTime += time;
		cv::Mat currentFrame2 = currentFrame.clone();
		currentFrame2 = dsst.visualize(rect, currentFrame2);
		putText(currentFrame2, dsst.intToStr(frameCnt, "", 0, ""), Point(10, 10), 1, 1, Scalar(0, 255, 0), 3);
